...
class AuthController {
  ...
  static Future<Map<String, dynamic>?> signup(
    ...
    if (profileImage != null) {
      String extension = profileImage.path
          .split('.')
          .last
    ...
    try {
      ...
      if (response.statusCode == 201) {
        ...
        logger.severe(
            '[STATUS CODE]: HTTP ${response.statusCode} ${response.reasonPhrase}'); // show log for debugging
        ...
        try {
          ...
          throw Exception(
              'Response: $responseData | Status code: ${response.statusCode}_${response.reasonPhrase}');
        } catch (err) {
          throw Exception(
              'Failed to parse error response: $err'); // if response couldn't be parsed
        }
      }
    } on TimeoutException catch (timeoutError) {
      isLoading(false);
  ...
  Future<void> logout() async {
    try {
      ...
      if (response.statusCode == 200) {
        ...
        final RxMap<String, dynamic> errorMessage = {
          "title": "Error",
      ...
      RxMap<String, dynamic> errorMessage =
          {"title": "Error", "message": "$e", "type": ContentType.failure}.obs;
      CustomSnackbar(snackbarMessage: errorMessage);
    }
  }
}
